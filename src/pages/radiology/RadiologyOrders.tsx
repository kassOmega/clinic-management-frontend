...
import { IconEye, IconScan } from "../../components/icons";
    ...
    <Card>
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <div className="text-right">
          <Badge variant="purple">Radiology Only</Badge>
        </div>
      </div>
...
export default function RadiologyOrders() {
  ...
  const { data: orders = [] } = useQuery<InvestigationOrder[]>({
    queryKey: ["orders", "radiology", "queue"],
    queryFn: api.getRadiologyQueue,
  });
  ...
  const processOrder = (order: InvestigationOrder) => {
    ...
    const testItems: TestItem[] = radioTests.map((test) => ({
      test,
      isCompleted: test.status === "COMPLETED",
      isInProgress: test.status === "IN_PROGRESS",
    }));

    const allCompleted = radioTests.every((t) => t.status === "COMPLETED");

    const total = radioTests.reduce((sum, t) => sum + t.price, 0);

    const patientStatus: PatientStatus | string = allCompleted
      ? order.status
      : (patient?.status ?? "IN_RADIOLOGY");
  ...
  const handleEnterResults = (orderId: number) => {
    navigate(`/radiology/results?orderId=${orderId}`);
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          {orders.length} order{orders.length !== 1 ? "s" : ""} with pending
          radiology tests
        </p>
      </div>
