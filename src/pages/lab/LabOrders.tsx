...
export default function LabOrders() {
  ...
  // Orders that have lab tests and are confirmed for processing
  const labOrders = orders.filter(
    (o) =>
      o.tests.some((t) => t.type === "lab") &&
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          {labOrders.length} orders with lab tests
        </p>
      </div>
      ...
      {labOrders.length === 0 ? (
        ...
        <div className="space-y-4">
          {labOrders.map((order) => {
            ...
            const allLabDone = labTests.every((t) => t.status === "COMPLETED");
                ...
                <div className="flex justify-end gap-2">
                  {!allLabDone && (
                    <Button
                      size="sm"
                      onClick={() =>
                        navigate(`/lab/results?orderId=${order.id}`)
                      }
                    >
                      Enter Results
                    </Button>
